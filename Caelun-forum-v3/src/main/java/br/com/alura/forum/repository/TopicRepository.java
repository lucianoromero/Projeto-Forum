...
public interface TopicRepository extends Repository<Topic, Long>, JpaSpecificationExecutor<Topic> {
	...
	int countLastWeekTopicsByCategoryId(@Param("categoryId") Long categoryId, @Param("lastWeek") Instant lastWeek);
	...
	//TODO findOpenTopicsByCategory()
	@Query("select new br.com.alura.forum.model.OpenTopicByCategory("
			+ "t.course.subcategory.category.name as categoryName, " + "count(t) as topicCount, "
			+ "now() as instant) from Topic t " + "where t.status = 'NOT_ANSWERED' "
